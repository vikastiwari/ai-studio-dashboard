...
const TopNav = ({ onOpenCommandPalette, onOpenModal }) => {
  ...
  const handleSearchSubmit = (e) => {
    if (e.key === 'Enter' && searchQuery.trim() !== '') {
      ...
      const responses = [
        "Analyzing orchestration metrics... Currently 4 nodes are idle. To optimize L4 utilization, I recommend scaling down the Spot VM cluster.",
        "Based on your query, the Wan 2.5 I2V API costs approximately $0.04 per generation at 1080p.",
        "I've checked the LangGraph state. The True Crime pipeline is currently waiting for the EchoMimic audio synthesis node.",
        "Global telemetry shows a 12% CPU overhead. This is well within optimal operating parameters for the Autonomous AI Studio."
      ];
              ...
              {isAlertsOpen && (
                ...
                >
                  ...
                  <div style={{ padding: '0.5rem' }}>
                    <DropdownItem icon={<AlertCircle color="var(--accent-magenta)" size={16} />} title="Spot VM Preemption Risk" desc="L4 cluster utilization spiked." />
                    <DropdownItem icon={<CheckCircle2 color="#00e676" size={16} />} title="Synthesis Complete" desc="True Crime pipeline finished." />
                    <DropdownItem icon={<Activity color="var(--accent-cyan)" size={16} />} title="FinOps Alert" desc="Daily budget crossed $1.50." />
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
              ...
              {isSettingsOpen && (
                ...
                >
                  <div style={{ padding: '0.5rem' }}>
                    ...
                    <DropdownItem icon={<Activity size={16} />} title="Performance" desc="Toggle WebGL acceleration" onClick={() => { playMenuClick(); onOpenModal('performance'); }} />
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
